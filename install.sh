...
main() {
    while true; do
        ...
        printf "${CYAN}|${NC}  ${BOLD}%-$(($line_width-2))s${NC}${CYAN}|${NC}\n" "服务控制"
        echo -e "${CYAN}+$(printf '-%.0s' $(seq 1 $left_col))+$(printf '-%.0s' $(seq 1 $right_col))+${NC}"
        printf "${CYAN}|${NC}  ${CYAN}[1]${NC} %-$(($left_col-6))s${CYAN}|${NC}  ${CYAN}[5]${NC} %-$(($right_col-6))s${CYAN}|${NC}\n" "查看状态" "实时日志"
        printf "${CYAN}|${NC}  ${CYAN}[2]${NC} %-$(($left_col-6))s${CYAN}|${NC}  ${CYAN}[6]${NC} %-$(($right_col-6))s${CYAN}|${NC}\n" "启动服务" "最近日志"
        printf "${CYAN}|${NC}  ${CYAN}[3]${NC} %-$(($left_col-6))s${CYAN}|${NC}  ${GREEN}[7]${NC} %-$(($right_col-6))s${CYAN}|${NC}\n" "停止服务" "手动推送"
        printf "${CYAN}|${NC}  ${CYAN}[4]${NC} %-$(($left_col-6))s${CYAN}|${NC}  ${CYAN}[8]${NC} %-$(($right_col-6))s${CYAN}|${NC}\n" "重启服务" "查看配置"
        echo -e "${CYAN}+$(printf '-%.0s' $(seq 1 $left_col))+$(printf '-%.0s' $(seq 1 $right_col))+${NC}"
        printf "${CYAN}|${NC}  ${BOLD}%-$(($line_width-2))s${NC}${CYAN}|${NC}\n" "系统管理"
        echo -e "${CYAN}+$(printf '=%.0s' $(seq 1 $line_width))+${NC}"
